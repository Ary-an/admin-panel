...
class StorageInfo extends React.Component {
	...
	getChartData = () => {
		const getCharts = JSON.parse(localStorage["backEndData"]).dasbhoardPage
			.storage;
		...
		tempData.labels = [...mainLabels].map((item, pos) => {
			return (
				...
				`${item.slice(1)} Storage (${dependentData[pos]})`
			);
		});

		tempData.datasets[0].data = [...dependentData];
		tempData.datasets[0].backgroundColor = ["#a8d582", "#4ed6b8", "#f7604d"];
