...
class ProductBlock extends React.Component {
	state = {
		productData: JSON.parse(localStorage["backEndData"]).productsPage.products,
		checkedItems: [],
	};
	...
	removeMultipleElements = () => {
		...
		const arrayWithValuesRemoved = tempArr.filter(
			(item, i) => !indexSet.has(i)
		);
	...
	render() {
			...
			return (
				<tr className="mainRow" key={pos + 1}>
					...
					<td className="deleteIcon">
						<a
							href="/"
							onClick={(e) => this.removeProduct(pos, e)}
		...
		return (
			<div className="product-block">
				...
				<button
					onClick={this.removeMultipleElements}
