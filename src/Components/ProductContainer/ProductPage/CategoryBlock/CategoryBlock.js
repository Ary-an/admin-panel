...
class CategoryBlock extends React.Component {
	state = {
		categoryList: JSON.parse(localStorage["backEndData"]).productsPage
			.categories,
	...
	render() {
		const data = JSON.parse(localStorage["backEndData"]).productsPage
			.categories;
			...
			return (
				<tr key={pos + 1}>
					...
					<td className="text-center">
						<a
							onClick={(e) => this.removeCategory(pos, e)}
							href="/"
